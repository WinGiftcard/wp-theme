    ...
    function isReadyForFire($event)
    {
        switch ($event) {
            ...
            case 'woo_FirstTimeBuyer': {
                $status = is_order_received_page() && PYS()->getOption( 'woo_FirstTimeBuyer_enabled' ) &&
                    wooIsRequestContainOrderId() && !has_user_orders();
                return $status;
            }
            case 'woo_ReturingCustomer':{
                $status = is_order_received_page() && PYS()->getOption('woo_ReturingCustomer_enabled') &&
                    wooIsRequestContainOrderId() && has_user_orders();
                return $status;
            }
            case 'woo_purchase' : {
                $status = PYS()->getOption( 'woo_purchase_enabled' ) // if purchase enable by plugin settings
                    && (is_order_received_page()  //if is order review(success) page
