...
class GA extends Settings implements Pixel {
    ...
    public function outputOptimizeSnippet() {
        ...
        if ( $this->configured() && $this->getOption( 'optimize_enabled' ) && ! empty( $optimize_id ) && !apply_filters( 'pys_disable_ga_optimize_by_gdpr', false)) {
            $optimizeError ="";
            ob_start();
...
// Обновление куки select_prod_list с адресом текущей страницы
            ...
            $_SESSION['select_prod_list'] = json_encode($productlist);
        }
